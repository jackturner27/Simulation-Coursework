...
public class HPP {
        ...
        for(int iter = 0 ; iter < NITER ; iter++) {
            ...
            for(int i = 0; i < NX ; i++) { 
                for(int j = 0; j < NY ; j++) { 
                    ...
                    if(fin[i][j][0] && fin[i][j][1] && !(fin[i][j][2] || fin[i][j][3]) ) {
                    	fout[i][j][0] = false;
                    	fout[i][j][1] = false;
                    	fout[i][j][2] = true;
                    	fout[i][j][3] = true;
                    } else if(fin[i][j][2] && fin[i][j][3] && !(fin[i][j][0] || fin[i][j][1]) ) {
                    	fout[i][j][0] = true;
                    	fout[i][j][1] = true;
                    	fout[i][j][2] = false;
                    	fout[i][j][3] = false;
                    } else {
                    	fout[i][j][0] = fin[i][j][0];
                    	fout[i][j][1] = fin[i][j][1];
                    	fout[i][j][2] = fin[i][j][2];
                    	fout[i][j][3] = fin[i][j][3];
                    }
    ...
    static class Display extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            for(int i = 0 ; i < NX ; i++) {
                for(int j = 0 ; j < NY ; j++) {
                    ...
                    int d = calculateDensity(fin[i][j]);
                    switch(d) {
                    	case 0:
                    		g.setColor(Color.WHITE);
                    		break;
        ...
        public int calculateDensity(boolean[] values) {
        	
        	int i = 0;
        	
        	//convert boolean values into integer and add them
        	i += values[0] ? 1 : 0;
        	i += values[1] ? 1 : 0;
        	i += values[2] ? 1 : 0;
        	i += values[3] ? 1 : 0;
